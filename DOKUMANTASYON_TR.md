...
- negatif array index normalizasyonu: `arr[-1]` son eleman, `arr[-2]` sondan ikinci eleman olur; sabit taşmalar diagnostic üretir
...
Codegen bugün sabit boyutlu array'lerde `arr[i]` ve `arr[i:j:k]` formunu row-major lineer index'e indirir. Sabit index expression'ı array sınırını aşarsa semantic pass codegen'e geçmeden error üretir. Dinamik index expression'ları da desteklenir; runtime bounds check henüz yoktur.
