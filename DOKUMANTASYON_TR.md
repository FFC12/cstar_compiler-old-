...
- `struct Name { field; ... }`, `trait`, static `protocol`, `attribute`, `macro/directive`, `except`/`throw` ve `defer` MVP grammar/semantic hattı çalışır. `dynamic protocol`, `.=` runtime tag lowering, async/await ve bazı ileri proposal yüzeyleri controlled diagnostic verir.
...
- `scope_exit A -> B :: method();` normal `ret`, `throw` ve implicit fallthrough çıkışlarında cleanup edge'i üretir. Scope-exit method'u instance method olmalı, user argümanı almamalı ve fallible olmamalıdır. Fallible cleanup açık user code ile yapılmalıdır.
...
- `new` yalnız struct/resource allocation değildir; sized concrete storage allocation'dır. Primitive heap allocation çalışır: `int32^ n = new int32(7);`, `float64* f = shared new float64(1.0);`, `int32^? maybe = new? int32(7);`. Primitive `new` constructor/destructor çağırmaz; allocate + initializer store + drop/release free hattına iner. Explicit allocator formu `new(arena) int32(7)` payload storage'ı allocator domain'inden alır ve `drop` ile aynı allocator'a geri verir. `drop` primitive pointer'da yalnız `new` kaynaklı heap ownership sembolleri için legal kalır; stack alias serbest bırakma yüzeyi açılmaz.
...
`with Trait` static conformance'dır ve vtable üretmez. Runtime dispatch isteyen kod bunu açık ve uzun `dynamic Trait` ile ister. Kısa `dyn` alias'ı canonical değildir; sistem programlama dili için runtime maliyetin syntax'ta görünmesi tercih edilir. Mevcut compiler borrowed `dynamic Trait&` için named-value erase syntax'ını, vtable global üretimini ve dynamic method dispatch lowering'ini destekler. Trait hedefi ve kaynak struct conformance'ı semantic olarak doğrulanır:
...
- `dynamic Trait*`: shared trait object; C* shared handle retain/release edecektir, shared lowering henüz diagnostic üretir.
- `dynamic Trait^`: unique trait object; `dynamic move owned as Trait` ile unique heap pointer sahipliğini devralır.
...
- `dynamic move value as Trait`: source `T^` unique heap pointer ise unique trait object üretir, source pointer null yapılır ve dynamic object düşerken concrete destructor + free çalışır.
...
### 15.8 Async/await/except/throw/defer
...
- `except ErrorType :: ReturnType`, `throw Expr;` ve `defer { ... }` MVP olarak parse/type-check/codegen edilir. `throw`, `except` imzalı fonksiyon ister ve throw expression enum/defined-type olarak error type ile uyumlu olmalıdır. `defer` cleanup blokları reverse lexical order ile çıkış hattında çalışır; `throw` path'i de defer ve protocol scope-exit cleanup'larını çalıştırır. İlk lowering structured early-return'dür; platform exception ABI ve error payload propagation henüz yoktur.
- Parser `async`/`await` yüzeyini bugün `CST1001` controlled proposal diagnostic ile durdurur; runtime scheduler/coroutine lowering henüz yoktur.
