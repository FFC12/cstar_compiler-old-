...
not_needed_type:
	...
	while(is(TokenKind::STAR) || is(TokenKind::XOR)) {
		size_t indirection_level = advancePointerType(this->currentTokenKind() == TokenKind::XOR ? true : false);
	...
	//This is an AssignmentExpr and then will take potentially a BinopExpr and then will  
	//take any other expressions like ConstExpr or *Expr .
	...
	if (is(TokenKind::COMMA)) {
		...
		//ASTNode ast = std::unique_ptr<VarAST>(new VarAST(name, std::move(rhs), TypeSpecifier::SPEC_I8, VisibilitySpecifier::VIS_EXPORT));
		auto ast = std::make_unique<VarAST>(name, std::move(rhs), TypeSpecifier::SPEC_I8, VisibilitySpecifier::VIS_EXPORT);
...
size_t CStarParser::advancePointerType(bool isUniquePtr) {
	...
	while(is(pointerType)){
		...
		if(pointerType == TokenKind::STAR && this->currentTokenKind() == TokenKind::XOR) {
			ErrorMessage("Next pointer type [^] is not matching with type before level [*]. This is not supported yet!");
		} else if(pointerType == TokenKind::XOR && this->currentTokenKind() == TokenKind::STAR) {
			ErrorMessage("Next pointer type [*] is not matching with type before level [^]. This is not supported yet!");
		}
